    ...
    return (
        <div className="mt-8 pb-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {/* Balance Card */}
                <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-blue-500/10 to-pink-500/10 dark:from-blue-500/20 dark:to-pink-500/20 border theme-border rounded-3xl p-8 backdrop-blur-xl flex flex-col justify-between min-h-[240px] relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-8 text-white/5 group-hover:text-blue-400/10 transition-colors">
                        <DollarSign size={120} />
                    </div>
                    <div>
                        <div className="text-sm font-mono theme-text-muted uppercase tracking-widest mb-2">Total Balance</div>
                        <div className="text-5xl font-black italic">${balance.toFixed(2)}</div>
                    </div>
                    <div className="flex gap-4 mt-8">
                        <button 
                            onClick={() => setCurrentPage?.('wallet')}
                            className="px-6 py-3 theme-button-accent font-bold rounded-2xl transition-all flex items-center gap-2"
                        >
                            Withdraw <ArrowUpRight size={18} />
                        </button>
                        <button 
                            onClick={() => setCurrentPage?.('wallet')}
                            className="px-6 py-3 theme-button-secondary font-bold rounded-2xl transition-all"
                        >
                            Transactions
                        </button>
                    </div>
                ...
                {/* Level Card */}
                <div className="theme-card border-blue-500/30 rounded-3xl p-8 flex flex-col justify-center items-center text-center relative overflow-hidden">
                    ...
                    <div className="text-sm font-bold theme-text-secondary uppercase">{getLevelTitle(level)}</div>
                    <div className="w-full theme-bg-surface h-1.5 rounded-full mt-4 overflow-hidden">
                        <div className="bg-blue-500 h-full shadow-[0_0_10px_rgba(0,85,255,0.5)]" style={{ width: `${progressPercent}%` }} />
                    </div>
                    <div className="text-[10px] theme-text-muted mt-2 font-mono italic">{xpRemaining} XP to Level {level + 1}</div>
                </div>
            </div>

            <h2 className="text-xl font-bold mb-6 italic">Creator Analytics</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    ...
                    <div key={i} className="theme-card rounded-2xl p-6 hover:theme-border-secondary transition-all">
                        <div className="flex justify-between items-start mb-4">
                            <div className={`p-2 rounded-xl theme-bg-surface ${stat.color}`}>
                                <stat.icon size={20} />
                            </div>
                        ...
                        <div className="theme-text-muted text-xs font-mono uppercase tracking-widest mb-1">{stat.label}</div>
            ...
            {/* Subscription Plans */}
            <div className="mt-12 theme-card rounded-3xl p-8">
                ...
                <div className="space-y-4">
                    {[
                        ...
                        <div key={i} className="flex justify-between items-center p-6 border theme-border rounded-2xl hover:bg-black/5 dark:hover:bg-white/5 transition-all">
                            <div>
                                <div className="font-bold">{plan.name}</div>
                                <div className="text-xs theme-text-muted font-mono italic">{plan.benefits}</div>
                            </div>
                            <div className="text-right">
                                <div className="font-bold text-blue-400">{plan.price}</div>
                                <div className="text-[10px] theme-text-muted uppercase tracking-widest">Active</div>
                            </div>
                        </div>
                    ))}
                </div>
